...
### ✨ 交互体验优化
#### 1. 精确的 Tooltip 触发区域
- **问题修复**：之前在“掉落历史”或“分配装备”列表中，鼠标悬停在整行（包括空白处、[T9] 标签、[BOE] 标签或 Boss 名字）都会显示装备的 Tooltip，这在浏览时会造成视觉干扰。
- **优化方案**：重构了列表渲染逻辑，现在装备 Tooltip **仅在鼠标精确悬停在装备链接（Item Link）上时**才会显示。
- **涉及界面**：
  - 掉落历史记录 (Loot History)
  - 装备分配列表 (Loot Selection)
  - 手动添加装备列表 (Manual Add)

#### 2. 代码重构
- 优化了 `LootHistory.lua` 和 `RaidLootCounter.lua` 中的滚动更新逻辑。
- 采用动态计算文本宽度的方式，将鼠标交互区域 (HitBox) 精确对齐到装备链接文本上。
